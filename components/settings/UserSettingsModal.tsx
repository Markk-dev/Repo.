...
function SwipeableSettingsRow({
  ...
  return (
    <div className={`settings-row-wrapper ${clickable ? 'clickable' : ''}`}>
      ...
      <div
        className={`settings-row ${clickable ? 'settings-row-clickable' : ''} ${!actionButton ? 'settings-row-full' : ''
          }`}
        style={{
...
interface SwipeableGoogleMfaCardProps {
  googleConnected: boolean;
  googleEmail: string | null;
  googleEmailRevealed: boolean;
  onToggleEmailReveal: () => void;
  socialLoading: string | null;
  onToggleGoogle: () => void;
  employeeId?: string;
}

function SwipeableGoogleMfaCard({
  googleConnected,
  googleEmail,
  googleEmailRevealed,
  onToggleEmailReveal,
  ...
  onToggleGoogle,
  employeeId,
}: SwipeableGoogleMfaCardProps) {
  const [offsetX, setOffsetX] = useState(0);
  const [isDragging, setIsDragging] = useState(false);
  const touchStartX = useRef<number | null>(null);
  const initialOffset = useRef<number>(0);
  const maxRevealOffset = -95;
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (touchStartX.current === null || !googleConnected) return;
    const currentX = e.touches[0].clientX;
    const diff = currentX - touchStartX.current;
    let nextOffset = initialOffset.current + diff;
    if (nextOffset > 0) nextOffset = 0;
  ...
  const emailDisplay = googleEmailRevealed
    ? (googleEmail || `${employeeId?.toLowerCase() || 'user'}@gmail.com`)
    : '••••••••••••••••@gmail.com';
  ...
  return (
    <div className="settings-row-wrapper">
      ...
      >
        <div className="mfa-provider-top-row">
          ...
          <div className="mfa-provider-info">
            ...
            <span className="mfa-provider-desc">
              {googleConnected ? (
                ...
                'Link Google for emergency recovery'
              )}
            </span>
          </div>
        </div>
...
export function UserSettingsModal({
  ...
  // Mobile drawer drag-to-dismiss gesture state
              ...
              <div className="discord-devices-scroll">
                ...
                <section className="discord-device-section">
                  ...
                  <div className="mfa-provider-list">
                    <SwipeableGoogleMfaCard
                      googleConnected={googleConnected}
                      googleEmail={googleEmail}
                      googleEmailRevealed={googleEmailRevealed}
                      onToggleEmailReveal={() => setGoogleEmailRevealed(!googleEmailRevealed)}
                  ...
                  <div className="mfa-info-tip-box">
                    <p className="mfa-info-tip-text">
                      <em>Note: Linking Google enables emergency identity verification if you lose access to your school credentials.</em>
                    </p>
                  </div>
                </section>
              </div>
